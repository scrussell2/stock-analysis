...
he code that was written for the analysis of the green stock investment distribution proved to be extremely useful.  Steve's parents wanted to invest in green energy and specifically a ticker, DQ.  Unfortunately in the year 2018, DQ suffered the worst return for the year.  Based on the overall returns of multiple different tickers, we can see in this analysis that the companies to favor investment were the tickers of ENPH and RUN.  In the original "AllStocksAnalysis" and the refactored "AllStocksAnalysisRefactored" the end data was exactly the same but the time it took to run was different.

To answer directly on performace after refactoring, there was significant improvement between the original and refactored code by a **147%** difference between the two codes. The original code ran at **0.765625** seconds, and the refactored at **0.1171875** seconds which can be seen in **Figure 1**.  In the below figure, please note that the refactored code also was able to incorporate more complexity such as formatting and highlighting of cells while gaining significant improvement in the efficiency of processing the data.  How this was accomplished by using the variable called 'tickerIndex' and incorporating its use to hold data and distribute the stored data efficiently when called upon within the refactored script.  Another significant note is the use of arrays throughout the refactored script which can be seen in the below **Figure 2** and **Figure 3** comparisons.  While it may not look like a significant amount of change has occurred, the success is in how the program uses the memory of the given device in shorter more relevant bursts.

...
The code that was written for the green stocks analysis proved to be extremely useful.  Steve's parents wanted to invest into green energy and specifically a ticker, DQ.  Unfortuneatly in the year of 2018, DQ suffered the worst return for the year.  Based on the overall returns of multiple different tickers, we can see in this analysis that the companies to favor for investment were the tickers of ENPH and RUN.  In the original "AllStocksAnalysis" and the refactored "AllStocksAnalysisRefactored" the end data was exactly the same but the time it took to run was different.
